...
{
    public class SqlDbRetryPolicy : IRetryPolicy
    {
        ...
        {
            _sqlRetryPolicy.Execute(action.Invoke);
        }

        public TResult Execute<TResult>(Func<TResult> action)
        {
            return _sqlRetryPolicy.Execute(action.Invoke);
        }

        private static bool AnyRetryableError(SqlException exception)
        {
            return exception.Errors.OfType<SqlError>().Any(ShouldRetryOn);
        }
        ...
        private static bool ShouldRetryOn(SqlError error)
        {
            switch (error.Number)
            {
